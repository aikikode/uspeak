...
class Recognizer(AudioSource):
    ...
    def samples_to_flac(self, source, frame_data):
        ...
        if flac_converter is None:  # flac utility is not installed
            if system == "Windows" and platform.machine() in ["i386", "x86", "x86_64",
                ...
                raise OSError(
                    "FLAC conversion utility not available - consider installing the FLAC command line application using `brew install flac` or your operating system's equivalent")
    ...
    def listen(self, source, timeout=None):
        ...
        pause_buffer_count = int(math.ceil(
            self.pause_threshold / seconds_per_buffer))  # number of buffers of quiet audio before the phrase is complete
        quiet_buffer_count = int(math.ceil(
    ...
    def recognize(self, audio_data, show_all=False):
        ...
        self.request = Request(url, data=audio_data.data,
